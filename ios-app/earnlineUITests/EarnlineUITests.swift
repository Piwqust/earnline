...
final class EarnlineUITests: XCTestCase {
    ...
    func testAddIncomeUsesTheInlineComposerFromTheLedgerMenu() {
        let app = launchApp(["-demoLedger"])
        ...
        let income = app.buttons["Income"]
        XCTAssertTrue(income.waitForExistence(timeout: 3))
        income.tap()

        let client = app.buttons["Acme Studio"].firstMatch
        XCTAssertTrue(client.waitForExistence(timeout: 3))
        client.tap()

        let amount = app.textFields["composer.amount"]
        let project = app.textFields["composer.project"]
        let task = app.textFields["composer.task"]
        ...
        amount.typeText("275")
        task.tap()
        task.typeText("Inline composer regression")
        submit.tap()

        let newEntry = app.descendants(matching: .any).matching(
            NSPredicate(format: "identifier BEGINSWITH %@", "entry.row.")
        ).firstMatch
        XCTAssertTrue(newEntry.waitForExistence(timeout: 5))
        XCTAssertTrue(newEntry.label.contains("Inline composer regression"))
    }
