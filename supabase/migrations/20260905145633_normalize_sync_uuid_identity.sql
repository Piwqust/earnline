...
begin
  ...
  for item in select value from jsonb_array_elements(p_rows) loop
    ...
    index := index + 1;
    -- Swift encodes UUIDs in uppercase; Postgres uuid::text is lowercase.
    -- Compare canonical UUID identity while keeping workspace text case-sensitive.
    key_value := case when key_column = 'id'
      then (item->>key_column)::uuid::text
      else item->>key_column end;
    remote_stamp := null;
